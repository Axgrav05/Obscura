...
| **Billy** | Defined Tower auth middleware plan for Obscura: validate `Authorization: Bearer <token>` against `OBSCURA_API_KEY` (env var), return `401` when missing/malformed, return `403` when invalid, and use constant-time comparison for token validation. | Implement Tower middleware for auth (**OBS-22a**).<br>Add constant-time key comparison (**OBS-22b**).<br>Write tests for 401, 403, and 200 paths (**OBS-22c**). | No issues encountered | 5hrs |
